...
if(trim($_POST['roomTemplateItemsId']) != '')
{
	$array_room_template_estimates = $room_template_estimates_obj->getAll("WHERE room_template_items_id = ".$_POST['roomTemplateItemsId'] ." ORDER BY item_name");
	
	if(count($array_room_template_estimates) > 0)
	{
		$result['success'] = true;
		//$result['message'] = $array_room_template_estimates;
		
		$result['message'] = '<div id="accordion" class="panel-group">';
		for($i=0;$i<count($array_room_template_estimates);$i++)
		{
			$result['message'] .= '<div class="panel panel-default">
				<div class="panel-heading">
					 <h4 class="panel-title">
						<a href="#panel'.($i+1).'" data-parent="#accordion" data-toggle="collapse" class="accordion-toggle collapsed"><i class="glyphicon glyphicon-plus"></i>'.$array_room_template_estimates[$i]['item_name'].'</a> 
					</h4>
					...
					<div class="panel-body">
						 <div class="estimatestemplate_row">			
							<div class="estimatestemplate_left">
								<input type="text" name="item_name_'.$i.'_e" id="item_name_'.$i.'_e" class="item_name" value="'.$array_room_template_estimates[$i]['item_name'].'" />
							</div>
							<div class="estimatestemplate_middle">
								$ <input type="text" name="price_per_unit_'.$i.'_e" id="price_per_unit_'.$i.'_e" class="price_per_unit" value="'.$array_room_template_estimates[$i]['price_per_unit'].'" />
							</div>
							<div class="estimatestemplate_right">
								...
								{
									if($array_room_template_estimates[$i]['unit_of_measure'] == $units_array[$j]['id'])
									{
