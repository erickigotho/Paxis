...
if(trim($_REQUEST['id']) != '') 
{
	...
	$array_properties = $community_properties_object->getOne();
}
?>
<div id="community_left">
	...
	<input type="hidden" id="property_id" name="property_id" value="<?php echo $_REQUEST['id'] ?>" />
		...
		<div class="pull-left"><h4>Edit Community</h4></div>
		...
		<div class="control-group">
			...
			<div class="controls">
                ...
                <option value="2"<?php if($array_properties['property_type'] == 2) print " selected='selected'"; ?>>Office</option>
                <option value="3"<?php if($array_properties['property_type'] == 3) print " selected='selected'"; ?>>Multifamily Interiors</option>
                <option value="4"<?php if($array_properties['property_type'] == 4) print " selected='selected'"; ?>>Industrial</option>
				</select>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" name="city" id="city" class="form-control" placeholder="City" data-validation="required" data-validation-error-msg="City is a required field." value="<?php print $array_properties['city']; ?>" />
			</div>
		</div>
		<div class="control-group">
			<label for="address" class="control-label">Zip Code</label>
			<div class="controls">
				<input type="text" name="zip" id="zip" class="form-control" placeholder="Zip Code" value="<?php print $array_properties['zip']; ?>" data-validation="required" data-validation-error-msg="Zip code is a required field."  />
			</div>
		</div>
		<div class="control-group">
        ...
        <div class="control-group">
			...
			<div class="controls">
				<textarea name="emailList" id="emailList" rows="3" placeholder="email_1@abc.com, email_2@abc.com, email_3@abc.com" data-validation="required" data-validation-error-msg="Enter email address's in comma-separated format."><?php print $array_properties['emails']; ?></textarea>
			</div>
		</div>
        <div class="control-group">
		...
		<div class="pull-right"><button class="btn btn-warning" type="submit">Save Changes</button> &nbsp; <a href="community.html" class="btn btn-default">Cancel</a>
			...
			<br /><br />
			<a class="btn btn-warning" href="#createPropertiesModal" data-toggle="modal">Create Properties</a>
	...
	<div id="rooms-wrapper" class="accordion">
		<?php
			...
			$community_reports_array = $community_reports_object->getAll("WHERE property_id = ".$_REQUEST['id']);
			...
			{ 
				...
				$community_report_rooms_array = $community_report_rooms_object->getAll("WHERE report_id = ".$community_reports_array['id'] ." ORDER BY id");
				
				$overal_room_array = array();
				$array_room_items = array();
				
				for($i=0;$i<count($community_report_rooms_array);$i++)
				{
					if(isset($community_reports_array[$i]['id']))
						$community_report_room_item_comments_array = $community_report_room_item_comments->getAllWithId_default("WHERE report_id = ".$community_reports_array[$i]['id'],"room_item_id");
					
					$array_rooms["roomId"] = $community_report_rooms_array[$i]['id'];
					$array_rooms["roomTemplateId"] = $community_report_rooms_array[$i]['room_template_id'];
					$array_rooms["roomName"] = $community_report_rooms_array[$i]['name'];
					$array_rooms["items"] = array();
					
					$community_report_room_items_array = $community_report_room_items_object->getAll("WHERE room_id = ".$community_report_rooms_array[$i]['id']." ORDER BY room_template_item_id");
					
					for($j=0;$j<count($community_report_room_items_array);$j++)
					{
						$array_room_items['itemId'] = $community_report_room_items_array[$j]['id'];
						$array_room_items['itemTemplateId'] = $community_report_room_items_array[$j]['room_template_item_id'];
						$array_room_items['itemName'] = $community_report_room_items_array[$j]['name'];
						$array_room_items['statusId'] = $community_report_room_items_array[$j]['status_id'];
						$array_room_items['work_category_id'] = $community_report_room_items_array[$j]['work_category_id'];
					
						$array_room_items['statusClass'] = $array_report_status[$community_report_room_items_array[$j]['status_id']]['class'];
						$array_room_items['statusName'] = $array_report_status[$community_report_room_items_array[$j]['status_id']]['name'];
						...
						if(count($community_report_room_item_comments_array) > 0)
						{
							...
							if(trim($community_report_room_item_comments_array[$community_report_room_items_array[$j]['id']]['comment']) != '')
							{
								$comment_array['comment'] = $community_report_room_item_comments_array[$community_report_room_items_array[$j]['id']]['comment'];
								$comment_array['commentDate'] = $community_report_room_item_comments_array[$community_report_room_items_array[$j]['id']]['date'];
	...
	<?php
		...
		$community_subcontractors_assign_array = $community_subcontractors_assign_object->getAll("WHERE property_id = ".$_REQUEST['id']);
		
		$community_subcontractors_ids_assign_array = array();
		for($i=0;$i<count($community_subcontractors_assign_array);$i++)
		{
			$community_subcontractors_ids_assign_array[$community_subcontractors_assign_array[$i]['work_category_id']][] = $community_subcontractors_assign_array[$i]['sub_contractor_id'];
		}
	...
	window.onload = function() {
		...
		$.validate({	
			...
			onSuccess : function() {
				...
				$.ajax({
					url: baseNameElem.value + "/webservice/edit_community.php",
					type: "POST",
