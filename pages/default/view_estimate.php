		...
		for($i=0;$i<count($estimate_room_items_units_array);$i++)
		{
			...
			if($estimate_room_items_units_array[$i]['room_template_id'] != $estimate_room_items_units_array[$i+1]['room_template_id'] || $estimate_room_items_units_array[$i]['estimate_room_name'] != $estimate_room_items_units_array[$i+1]['estimate_room_name'] || !$estimate_room_items_units_array[$i+1]['room_template_id'])
			{
				...
				if($i == (count($estimate_room_items_units_array) -1))
				{
					...
					$estimatesEmailBody .= "<tr style='text-align:left;font:bold  Arial;color:#fff'><td colspan='3'>&nbsp;&nbsp;<strong>Total</strong>&nbsp;&nbsp;</td><td style='text-align:right;font:normal Arial;color:#fff'>&nbsp;&nbsp;$".number_format(number_format($total_cost, 2, '.', ''),2)."&nbsp;&nbsp;</td></tr>";
				}
				else
					$estimatesEmailBody .= "<tr><td colspan='4'>&nbsp;</td></tr>";
...
?>
	...
	<div class="pull-right"><a href="edit_property_estimate.html?propertyId=<?php echo $propertyArray['id']; ?>" class="btn btn-default b">Cancel</a></div>
    ...
    <?php
	if($estimatesArray['is_closed'] != 2 && $estimatesArray['is_closed'] != 1)
	{
		if(count($arrayOtherEstimates) <= 0)
		{
	?>
    <button class="btn btn-primary" type="submit"  id="submitReportBtn" onClick="sendEstimate();"><i class="icon-ok icon-white"></i> Resend Estimate</button> <button class="btn btn-primary" type="submit"  id="submitReportBtn" onClick="acceptEstimate();"><i class="icon-ok icon-white"></i> Accept Estimate</button> <button class="btn btn-primary" type="submit"  id="submitReportBtn" onClick="archiveEstimate();"><i class="icon-remove icon-white"></i> Archive Estimate</button>
    <?php
		}
	}
	?>
	...
	function acceptEstimate()
	{
		...
		if(accept)
		{
			...
			type: "POST",
			data: { 
					 propertyId:<?php print $estimatesArray['property_id']; ?>,estimates_id:<?php print $estimatesArray['id']; ?>
				} 
			}).done(function( response ) {
				if(response.success) {
	...
	function archiveEstimate()
	{
		...
		if(archive)
		{
			...
			type: "POST",
			data: { 
					 propertyId: <?php print $estimatesArray['property_id']; ?>,estimate_id:<?php print $estimatesArray['id']; ?>
				} 
			}).done(function( response ) {
				if(response.success) {
					//$("#currentEstimates").html(response.message);
