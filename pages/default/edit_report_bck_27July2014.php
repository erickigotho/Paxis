...
if($propertyId != 0 && $reportId != 0) {
	...
	$array_report_images_count = $report_images->getAll("WHERE property_id = ".$propertyId);
							...
							?>
								<?php
									...
									<div class="roomitem-group">
										<div class="roomitem-desc"><?php if(count($item['arrayRoomItemsUnits']) > 0) print "<strong>"; ?><?php echo $item['itemName']; ?><?php if(count($item['arrayRoomItemsUnits']) > 0) print "</strong>"; ?>
                                        </div>
										...
										<div class="roomitem-image-upload" id="roomItemImageContainer_<?php echo $tmpRoomIndex . '_' . $roomItemIndex;?>"<?php if($array_report_images[$item['itemId']]['count'] > 1)print " style='padding-right:10px;'"; ?>><?php echo (isset($array_report_images[$item['itemId']])? '<a rel="light_box_2" href="images/report_uploads/'.$array_report_images[$item['itemId']]['image_name'].'"><span class="icon-image-upload"></span>'.($array_report_images[$item['itemId']]['count'] > 1? '<span class="badge">'.$array_report_images[$item['itemId']]['count'].'</span></a>': '') :'<a rel="light_box" href=""><span class="icon-image-upload-test"></span></a>'); ?> </div>
                                        <?php
											if(trim($array_report_images[$item['itemId']]['extra_images']) != '')
											{
												...
												{
													print "<a rel='light_box_2' href='images/report_uploads/".$extraImagesArray[$i]."'><img src='' style='display:none;' /></a>";
												}
											}
										?>
										<div class="clearfix"></div>
											...
											?>
												...
												<?php
													...
													for($j=0;$j<count($item['image_thread']);$j++)
													{
														...
														{
														?>
                                                        	<a tabindex="1" rel="light_box_2" href="images/report_uploads/<?php print $item['image_thread'][$j]['imageName']; ?>"><img src="images/report_uploads/<?php print $item['image_thread'][$j]['imageName']; ?>" style="height:50px;" /></a>&nbsp;
                                                        <?php	
														}
													}
                                                ?>
									...
									if(count($item['arrayRoomItemsUnits']) > 0)
									{
										for($k=0;$k<count($item['arrayRoomItemsUnits']);$k++)
										{
									?>
                                    <div class="roomitem-group">
										<div class="roomitem-desc roomitem-desc_padding_left"><?php echo $item['arrayRoomItemsUnits'][$k]['item_name']; ?>
                                        </div>
										<div class="roomitem-action">
                                        	<?php
												if($_SESSION['user_type'] == 5 && ($item['statusId'] == 1 || $item['statusId'] == 4))
				...
				data: {reportId: <?php echo $reportId; ?>
					...
					for(var i=0; i<existingRooms.length; i++) {
						...
						for(var j=0; j < existingRooms[i].items.length; j++) {
							...
							{
								arrRoomItems.push({id: existingRooms[i].items[j].itemId, name: existingRooms[i].items[j].itemName, statusId: existingRooms[i].items[j].statusId, comment: existingRooms[i].items[j].comments[0].comment});
							}
							else
							{
		...
		for(var i=0; i<existingRooms.length; i++) {
			...
			for(var j=0; j < existingRooms[i].items.length; j++) {
				...
				if(existingRooms[i].items[j].comments.length > 0)
				{
					...
					{
						arrRoomItems.push({id: existingRooms[i].items[j].itemId, name: existingRooms[i].items[j].itemName, statusId: existingRooms[i].items[j].statusId, comment: existingRooms[i].items[j].comments[0].comment, image:existingRooms[i].items[j].images[0].image_name});
					}
					else
					{		
						arrRoomItems.push({id: existingRooms[i].items[j].itemId, name: existingRooms[i].items[j].itemName, statusId: existingRooms[i].items[j].statusId, comment: existingRooms[i].items[j].comments[0].comment});
		...
		$("#statusModal").on('shown', function(){
					...
					if(existingImages.length > 0)
					{
						...
						for(var d=0; d < existingImages.length; d++) 
						{
							if(existingImages[d].roomItemId == 	existingComments[c].roomItemId)
							{
								...
								htmlComments += "<a href='images/report_uploads/"+existingImages[d].imageName+"' rel='light_box_2' tabindex='1'><img src='images/report_uploads/"+existingImages[d].imageName+"' style='height:50px;' /></a>&nbsp;";
			...
			/*if(existingImages.length > 0) {
				for(var c=0; c < existingImages.length; c++) {
					$("#existingImages").append("<tr><td>" + existingImages[c].imageUploadDate + "</td><td><a href='images/report_uploads/"+existingImages[c].imageName+"' rel='light_box_2' tabindex='1'><span class='icon-image-upload'></span></a></td><td> by: " + existingImages[c].firstName + " " + existingImages[c].lastName + "</td></tr>");
				}
			} else {
				$("#existingImages").html("");
	...
	function createUploader(){
		var uploader = new qq.FileUploader({
			...
			onComplete: function(id, fileName, responseJSON){
				$("#roomItemImageContainer_" + m_tempRoomIndex + "_" + m_tempRoomItemIndex).html('<a rel="light_box_2" href="images/report_uploads/'+responseJSON.filename+'" tabindex="1"><span class="icon-image-upload"></span></a>');
			},
