		...
		if ($stmt = $mysqli->prepare($sql)) 
		{
			...
			{	
				...
				$itemsSql = "SELECT
							...
							ORDER BY report_room_items.room_template_item_id,report_room_items.is_estimate
						";
				...
				if(count($roomItems) > 0)
				{
					...
					if(count($arrayRoomName)> 0)
					{
						...
						estimate_room_items_units.status_id,work_category_estimates.item_name, 
						...
						INNER JOIN work_category_estimates 
						ON work_category_estimates.id = estimate_room_items_units.work_category_estimates_id
						...
						if(count($arrComments) > 0)
						{
							for($i=0;$i<count($arrComments);$i++)
							{
								for($j=0;$j<count($roomItems);$j++)
								{
									...
									$arrComments[$i]['itemName'] == $roomItems[$j]["itemName"])
									{
